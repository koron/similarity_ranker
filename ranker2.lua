...
local function get_score(pivot_key, target_key, tmp_key)
    ...
    local count = redis.call('BITCOUNT', tmp_key)
    return count
end
...
local function ranking2(pivot_key, keys_key, score_key, tmp_key, limit, offset)
    ...
    local retval = redis.call('ZREVRANGE', score_key, 0, -1)
    redis.call('DEL', tmp_key)
    redis.call('DEL', score_key)
    return retval
end

return ranking2(KEYS[1], KEYS[2], KEYS[3], KEYS[4], ARGV[1], 100)
